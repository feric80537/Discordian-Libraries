...
public class DDate {
	public static String[] SEASON_NAME = { "error", "Chaos", "Discord",
			"Confusion", "Bureaucracy", "The Aftermath" };
	public static String[] SEASON_ABB = { "er", "Ch", "Dd", "Gr", "St", "Ta" };
	public static String[] DAY_NAME = { "St. Tib's Day", "Sweeetmorn",
			"Boomtime", "Pungenday", "Prickle-Prickle", "Setting Orange" };
	private static String[][] HOLYDAY = { { "Mungoday", "Chaoflux" },
			{ "Mojoday", "Discoflux" }, { "Syaday", "Confuflux" },
			{ "Zaraday", "Bureflux" }, { "Maladay", "Afflux" } };
	...
	public DDate(int gregYear, int gregMonth, int gregDay) {
		gDay = gregDay;
		gMonth = gregMonth;
		gYear = gregYear;
		calculateDates();
	...
	private void calculateDates() {
		...
		// System.out.println("gDay:" + gDay + " gMonth: " + gMonth + " gYear" +
		// gYear + " gYday: " + gYday);
		if (isLeap(gYear)) {
			if (gYday >= 61) {
				gYday = gYday - 1;
